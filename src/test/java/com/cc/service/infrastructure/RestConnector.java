package com.cc.service.infrastructure;
...
public class RestConnector {
	...
	public String bulidEntityCollectionUrl(String entityType){
	...
	private Response doHttp(String type, String url, String queryString, byte[] data, Map<String, String> headers, Map<String, String> cookies)
		...
		prepareHttpRequest (conn, headers, data, cookieString);
		conn.connect();
		Response ret = retrieveHtmlResponse(conn);
		
		updateCookies(ret);
		
		Exception failure = ret.getFailure();
		if (isCheckFailure() && failure != null){
			System.err.println("Error in calling service: " + url);
		}
	 ...
	 * @param cookieString
	 * 			the cookies data from clients side, such as lwsso, qxsession,
	 * 			jsession etc.
	...
	/**
	 * @param con
	 * 			that is already connected to its urlwith an http request, and
	...
	private Response retrieveHtmlResponse(HttpURLConnection con) throws Exception {
		...
		resp.setResponseHeaders(con.getHeaderFields());
